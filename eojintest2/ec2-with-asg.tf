...
resource "aws_launch_template" "ej_ec2_launch_templ_was" {
  ...
  network_interfaces {
    associate_public_ip_address = false
    # subnet_id                   = aws_subnet.ej_privsubnet_2a_was.id  # Multi-AZ 구성을 위해 주석, ASG에 Subnet 지정
...
resource "aws_autoscaling_group" "ej_asg_was" {
  ...
  # Connect to the target group
  target_group_arns = [aws_lb_target_group.ej_alb_tg_in.arn]

  vpc_zone_identifier = [ # Creating EC2 instances in private subnet
    aws_subnet.ej_privsubnet_2a_was.id, aws_subnet.ej_privsubnet_2b_was.id
  ]
